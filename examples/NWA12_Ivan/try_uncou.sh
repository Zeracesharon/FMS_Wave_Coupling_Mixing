...
NUM_TOT=1  ###overall duration/months
...
###modify data_table
fyear=$current_year
find . -type l -name "StokesDriftBands_*.nc" -exec rm {} \;
ln -sf INPUT/StokesDriftBands_${fyear}.nc ./
cat > "data_table" << EOF
...
#
"OCN" , "Usx1" , "Usx1" , "INPUT/StokesDriftBands_${fyear}.nc" , "bilinear" , 1.0
"OCN" , "Usy1" , "Usy1" , "INPUT/StokesDriftBands_${fyear}.nc" , "bilinear" , 1.0
"OCN" , "Usx2" , "Usx2" , "INPUT/StokesDriftBands_${fyear}.nc" , "bilinear" , 1.0
"OCN" , "Usy2" , "Usy2" , "INPUT/StokesDriftBands_${fyear}.nc" , "bilinear" , 1.0
"OCN" , "Usx3" , "Usx3" , "INPUT/StokesDriftBands_${fyear}.nc" , "bilinear" , 1.0
"OCN" , "Usy3" , "Usy3" , "INPUT/StokesDriftBands_${fyear}.nc" , "bilinear" , 1.0
"OCN" , "Usx4" , "Usx4" , "INPUT/StokesDriftBands_${fyear}.nc" , "bilinear" , 1.0
"OCN" , "Usy4" , "Usy4" , "INPUT/StokesDriftBands_${fyear}.nc" , "bilinear" , 1.0
"OCN" , "Usx5" , "Usx5" , "INPUT/StokesDriftBands_${fyear}.nc" , "bilinear" , 1.0
"OCN" , "Usy5" , "Usy5" , "INPUT/StokesDriftBands_${fyear}.nc" , "bilinear" , 1.0
"OCN" , "Usx6" , "Usx6" , "INPUT/StokesDriftBands_${fyear}.nc" , "bilinear" , 1.0
"OCN" , "Usy6" , "Usy6" , "INPUT/StokesDriftBands_${fyear}.nc" , "bilinear" , 1.0
EOF
...
ln -sf ../../stokes/StokesDriftBands_${fyear}.nc ./
...
new_line2="#override SURFBAND_FILENAME = "StokesDriftBands_${fyear}.nc""
        # Replace the line with the keyword
sed -i "/#override SURFBAND_FILENAME/c\\$new_line2" "MOM_override"
echo "Replaced the line with: $new_line2 in the file MOM_override."
cd ../
...
gcp -r ${start_y}${start_m}${start_d}_results gfdl:/archive/Qian.Xiao/Qian.Xiao/FMS_Wave_Coupling_GOTM_kapp/$current_folder/
...
if [[ $? -eq 0 ]]; then
  ...
  cp ${start_y}${start_m}${start_d}_results $current_folder/
  gcp -r $current_folder gfdl:/archive/Qian.Xiao/Qian.Xiao/FMS_Wave_Coupling_GOTM_kapp/
	  if [[ $? -eq 0 ]]; then
...
#	tar -czf ${end_y}${end_m}${end_d}_results.tar.gz ${end_y}${end_m}${end_d}_results
        gcp -r ${end_y}${end_m}${end_d}_results gfdl:/archive/Qian.Xiao/Qian.Xiao/FMS_Wave_Coupling_GOTM_kapp/$current_folder/
	if [[ $? -eq 0 ]]; then
	  echo "File transfer was successful."
	else
	  echo "File transfer failed.try to create a folder"
	  mkdir $current_folder
	  cp ${end_y}${end_m}${end_d}_results $current_folder/
	  gcp -r $current_folder gfdl:/archive/Qian.Xiao/Qian.Xiao/FMS_Wave_Coupling_GOTM_kapp/
	          if [[ $? -eq 0 ]]; then
			  echo "file transfer succedded"
	 	  else
	  ...
	  if [[ -e "./${end_y}${end_m}${end_d}_results" ]]; then
