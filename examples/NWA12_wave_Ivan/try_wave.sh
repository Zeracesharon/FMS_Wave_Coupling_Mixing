...
casename=twoway_Wave

rm restar*ww3 out*ww3
pushd INPUT/
ln -fs MOM_layout_20x30 MOM_layout
ln -fs MOM_layout_20x30 SIS_layout
popd
...
cd WW3/PostProc
	...
	../../../../build/ncrc6.intel23/ww3_ounf/REPRO/ww3_ounf > ../../log.ww3ounf
...
cp -rf INPUT/MOM_override results/
rm -rf ${start_y}${start_m}${start_d}*.nc

mv results ${start_y}${start_m}${start_d}_results_${casename}
#tar -czf ${start_y}${start_m}${start_d}_results.tar.gz ${start_y}${start_m}${start_d}_results
gcp -r ${start_y}${start_m}${start_d}_results_${casename} gfdl:/archive/Qian.Xiao/Qian.Xiao/FMS_Wave_Coupling_GOTM_kapp_C6/$current_folder/
...
if [[ $? -eq 0 ]]; then
  ...
  cp -rf ${start_y}${start_m}${start_d}_results_${casename} $current_folder/
  gcp -r $current_folder gfdl:/archive/Qian.Xiao/Qian.Xiao/FMS_Wave_Coupling_GOTM_kapp_C6/
	  if [[ $? -eq 0 ]]; then
...
#	fi
	...
	rm -rf ${end_y}${end_m}${end_d}*.nc
 	mv results ${end_y}${end_m}${end_d}_results_${casename}
	#tar -czf ${end_y}${end_m}${end_d}_results.tar.gz ${end_y}${end_m}${end_d}_results
        gcp -r ${end_y}${end_m}${end_d}_results_${casename} gfdl:/archive/Qian.Xiao/Qian.Xiao/FMS_Wave_Coupling_GOTM_kapp_C6/$current_folder/
	if [[ $? -eq 0 ]]; then
	  echo "File transfer was successful."
	else
	  echo "File transfer failed.try to create a folder"
	  mkdir $current_folder
	  cp -rf ${end_y}${end_m}${end_d}_results_${casename} $current_folder/
	  gcp -r $current_folder gfdl:/archive/Qian.Xiao/Qian.Xiao/FMS_Wave_Coupling_GOTM_kapp_C6/
	          if [[ $? -eq 0 ]]; then
			  echo "file transfer succedded"
	 	  else
	  ...
	  if [[ -e "./${end_y}${end_m}${end_d}_results_${casename}" ]]; then
	...
	rm -rf ${end_y}${end_m}${end_d}_results_${casename}
