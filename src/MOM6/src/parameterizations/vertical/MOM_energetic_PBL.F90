...
type, public :: energetic_PBL_CS ; private
  ...
  real :: LT_shear_frac_cap = 1.2
  real :: Ustar_min_thre=1.0e-4
...
! mixing.
  ...
  real :: zsum, hmin
  logical :: mstar_shear_ok, thick_ok,mstar_shear_cap
  integer :: k_H
  real :: Ri_lo, Ri_hi, a_lo, a_hi, w_shear,u0, Nfold, u_scale,arg
            ...
            ! --- reference LT on ePBL base to form fraction f0 (no shear rescale) ---
               ...
               if (CS%LT_rescale_by_shear) then
                ...
                (mstar_LT0 < 0.8 * mstar_total) .and. (min(mstar_total,mstar_e_local)> 1.0e-5)  )  then                       
                       ...
                       lt_scale = min(max(1.0, ratio_lt**CS%LT_rescale_gamma),CS%Shear_ePBL_LT_cap)
